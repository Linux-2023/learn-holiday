...
edge_index = torch.tensor(np.load('data/edge_index.npy'), dtype=torch.int64)
dif_s = torch.tensor(sparse.load_npz('data/dif_s.npz').toarray(), dtype=torch.float, device=DEVICE)
...
degree_index = torch.tensor(np.load('data/degree_index.npy'), dtype=torch.int64, device=DEVICE)
# 初始化模型
model = GIKT(
    num_question, num_skill, q_neighbors, s_neighbors, qs_table,degree_index,dif_s,
...
for epoch in range(params['epochs']):
    ...
    for fold, (train_indices, test_indices) in enumerate(k_fold.split(dataset)):
        ...
        for data in train_loader:
            ...
            y_hat = model(x, y_target, mask, edge_index)
    ...
    if epoch + 1 in [2000]:
        np.savetxt(f'chart_data/{time_now}_{epoch}_all.txt', y_label_all[:, :epoch_total])
        torch.save(model, f=f'model/{time_now}.pt')
        np.savetxt(f'chart_data/{time_now}_{epoch}_aver.txt', y_label_aver[:, :epoch+1])
        print("save")
output_file.close()