...
from .entity import Question, Skill, Answer,User


# 通过assist09数据集中的数据初始化mysql数据库
def create_data():
    data = pd.read_csv('C:/KT/Flask-BackEnd/app/alg/data/assist09_processed.csv', encoding='ISO-8859-1')
    question2idx = np.load('C:/KT/Flask-BackEnd/app/alg/data/question2idx.npy', allow_pickle=True).item()
    skill2idx = np.load('C:/KT/Flask-BackEnd/app/alg/data/skill2idx.npy', allow_pickle=True).item()
    user2idx = np.load('C:/KT/Flask-BackEnd/app/alg/data/user2idx.npy', allow_pickle=True).item()
