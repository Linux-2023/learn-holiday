...
class TOPIC(db.Model):
    ...
    answer = db.Column(db.Integer, nullable=False)  # 1为正确，0位错误

    def __repr__(self):
        return f'TOPIC[id: {self.id}, q_id: {self.q_id}, topic: {self.topic}, correct: {self.answer}]'

    # 实现下面两个方法就可以使用函数python内置函数：dict()
    def keys(self):
        return 'id', 'q_id', 'topic', 'answer'
